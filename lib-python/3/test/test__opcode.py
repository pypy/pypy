...
class OpcodeTests(unittest.TestCase):
    ...
    def test_stack_effect_jump(self):
        ...
        # CPython's FOR_ITER jump target is always END_FOR (pops 2), so the
        # compiler models the jump branch as pushing a value too (net +1);
        # in practice CPython's interpreter skips END_FOR and pops the
        # iterator directly on exhaustion (net -1) -- see the comment above
        # the "family(for_iter, ...)" block in CPython's Python/bytecodes.c.
        # PyPy has no END_FOR and its FOR_ITER pops the iterator directly on
        # exhaustion, so it reports the real (-1) effect for the jump branch.
        self.assertEqual(stack_effect(FOR_ITER, 0, jump=True),
                          1 if check_impl_detail(pypy=False) else -1)
