...
ffi.cdef("""
int prlimit(int pid, int resource, const void *new_limit, void *old_limit);
""")

lib = ffi.dlopen(find_library("c"))

if hasattr(lib, "prlimit"):
    ffi.cdef("""
...
$PRLIMIT_DEF
""".replace('$RLIMIT_CONSTS', ''.join(rlimit_consts)).replace('$PRLIMIT_DEF', prlimit_def))
