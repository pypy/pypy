...
class StatResultRepr(Repr):
    ...
    def redispatch_getfield(self, hop, index):
        rtyper = self.rtyper
        hop2 = hop.copy()
        v_tuple = hop.args[0].v
        v_index = const(index)
        spaceop = op.getitem(v_tuple, v_index)
        spaceop.result = hop.spaceop.result
        hop2.spaceop = spaceop
        s_index = rtyper.annotator.bookkeeper.immutablevalue(index)
        h_tuple = HopArg(v_tuple, self.s_tuple, self.r_tuple)
        h_index = HopArg(v_index, s_index, rtyper.getrepr(s_index))
