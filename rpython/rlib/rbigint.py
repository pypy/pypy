...
class rbigint(object):
    ...
    _immutable_ = True
    _immutable_fields_ = ["_digits[*]", "_size", "_sign"]
    ...
    def set_sign(self, sign):
        assert sign == 0 or sign == 1 or sign == -1
        self._sign = sign

    def scale_sign(self, sign):
        assert sign == 0 or sign == 1 or sign == -1
        self._sign *= sign
    ...
    def mul(self, other):
        ...
        result.set_sign(self.sign() * other.sign())
        return result
    ...
    def int_mul(self, iother):
        ...
        result.set_sign(self.sign() * othersign)
        return result
    ...
    def int_floordiv(self, iother):
        ...
        if mod != 0 and self.sign() * intsign(iother) == -1:
            if div.sign() == 0:
                return ONENEGATIVERBIGINT
            div = div.int_add(1)
        div.set_sign(self.sign() * intsign(iother))
        div._normalize()
        return div
...
def _k_lopsided_mul(a, b):
    ...
    while bsize > 0:
        ...
        if last_nonnull_index >= 0:
            bslice.set_size(last_nonnull_index + 1)
