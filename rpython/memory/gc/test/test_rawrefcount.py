...
class TestRawRefCount(BaseDirectGCTest):
    ...
    def _collect(self, major, expected_trigger=0):
        ...
        if major:
            ...
            space.gc._minor_collection()
        count1 = len(space.trigger)
        space.gc.rrc_invoke_callback()
        count2 = len(space.trigger)
    ...
    def _rawrefcount_pair(self, intval, is_light=False, is_pyobj=False,
        ...
        def check_alive(extra_refcount):
            ...
            p1ref = space.gc.rawrefcount_to_obj(r1addr)
            p1 = lltype.cast_opaque_ptr(lltype.Ptr(S), p1ref)
            assert p1.x == intval
            if not is_pyobj:
                assert space.gc.rawrefcount_from_obj(p1ref) == r1addr
            else:
                assert space.gc.rawrefcount_from_obj(p1ref) == llmemory.NULL
            return p1
    ...
    def test_rawrefcount_objects_basic(self, old):
        ...
        assert space.gc.rawrefcount_from_obj(p2ref) == llmemory.NULL
        assert space.gc.rawrefcount_to_obj(r2addr) == lltype.nullptr(
            llmemory.GCREF.TO)
    ...
    def test_rawrefcount_objects_collection_survives_from_raw(self, old):
        ...
        space.gc.check_no_more_rawrefcount_state()
        assert space.trigger == []
        assert space.gc.rawrefcount_next_dead() == llmemory.NULL
    ...
    def test_pypy_nonlight_survives_from_raw(self, old):
        ...
        assert r1.ob_pypy_link == 0
        r1addr = llmemory.cast_ptr_to_adr(r1)
        assert space.gc.rawrefcount_next_dead() == r1addr
        assert space.gc.rawrefcount_next_dead() == llmemory.NULL
        assert space.gc.rawrefcount_next_dead() == llmemory.NULL
        space.gc.check_no_more_rawrefcount_state()
        lltype.free(r1, flavor='raw')
