...
class OptIntAutoGenerated(object):
        ...
        if arg_0_int_and is not None:
            ...
            if b_arg_0_1.is_constant():
                ...
                if C_arg_0_1 == MININT:
                    ...
                    return
        return self.emit(op)
    _rule_names_int_is_zero = ['is_zero_true', 'is_zero_is_true', 'is_zero_is_zero']
    _rule_fired_int_is_zero = [0] * 3
    ...
    def optimize_INT_IS_ZERO(self, op):
        ...
        if intmask(b_arg_0.lower) > 0 or intmask(b_arg_0.upper) < 0 or intmask(b_arg_0.tvalue) != 0:
            ...
            return
        arg_0_int_is_true = self.optimizer.as_operation(arg_0, rop.INT_IS_TRUE)
        if arg_0_int_is_true is not None:
            arg_0_0 = get_box_replacement(arg_0_int_is_true.getarg(0))
            b_arg_0_0 = self.getintbound(arg_0_0)
            # is_zero_is_true: int_is_zero(int_is_true(x)) => int_is_zero(x)
            newop = self.replace_op_with(op, rop.INT_IS_ZERO, args=[arg_0_0])
            self.optimizer.send_extra_operation(newop)
            self._rule_fired_int_is_zero[1] += 1
            return
        else:
            arg_0_int_is_zero = self.optimizer.as_operation(arg_0, rop.INT_IS_ZERO)
            if arg_0_int_is_zero is not None:
                arg_0_0 = get_box_replacement(arg_0_int_is_zero.getarg(0))
                b_arg_0_0 = self.getintbound(arg_0_0)
                # is_zero_is_zero: int_is_zero(int_is_zero(x)) => int_is_true(x)
                newop = self.replace_op_with(op, rop.INT_IS_TRUE, args=[arg_0_0])
                self.optimizer.send_extra_operation(newop)
                self._rule_fired_int_is_zero[2] += 1
                return
        return self.emit(op)
    ...
    def optimize_INT_NEG(self, op):
        ...
        if arg_0_int_neg is not None:
            ...
            return
        return self.emit(op)
    _rule_names_jit_choose_i = ['choose_to_int_is_true', 'choose_is_no_choice', 'choose_false', 'choose_true', 'choose_self_left', 'choose_self_right']
    _rule_fired_jit_choose_i = [0] * 6
    _all_rules_fired.append(('jit_choose_i', _rule_names_jit_choose_i, _rule_fired_jit_choose_i))
    def optimize_JIT_CHOOSE_I(self, op):
        arg_0 = get_box_replacement(op.getarg(0))
        b_arg_0 = self.getintbound(arg_0)
        arg_1 = get_box_replacement(op.getarg(1))
        b_arg_1 = self.getintbound(arg_1)
        arg_2 = get_box_replacement(op.getarg(2))
        b_arg_2 = self.getintbound(arg_2)
        if b_arg_0.is_constant():
            C_arg_0 = b_arg_0.get_constant_int()
            # choose_false: jit_choose_i(0, x, y) => x
            if C_arg_0 == 0:
                self.make_equal_to(op, arg_1)
                self._rule_fired_jit_choose_i[2] += 1
                return
            # choose_true: jit_choose_i(1, x, y) => y
            if C_arg_0 == 1:
                self.make_equal_to(op, arg_2)
                self._rule_fired_jit_choose_i[3] += 1
                return
        # choose_is_no_choice: jit_choose_i(x, y, y) => y
        if self._eq(arg_2, b_arg_2, arg_1, b_arg_1):
            self.make_equal_to(op, arg_1)
            self._rule_fired_jit_choose_i[1] += 1
            return
        if b_arg_1.is_constant():
            C_arg_1 = b_arg_1.get_constant_int()
            if b_arg_2.is_constant():
                C_arg_2 = b_arg_2.get_constant_int()
                # choose_to_int_is_true: jit_choose_i(x, 0, 1) => int_is_true(x)
                if C_arg_1 == 0:
                    if C_arg_2 == 1:
                        newop = self.replace_op_with(op, rop.INT_IS_TRUE, args=[arg_0])
                        self.optimizer.send_extra_operation(newop)
                        self._rule_fired_jit_choose_i[0] += 1
                        return
        # choose_self_left: jit_choose_i(x, x, y) => jit_choose_i(x, 0, y)
        if self._eq(arg_1, b_arg_1, arg_0, b_arg_0):
            newop = self.replace_op_with(op, rop.JIT_CHOOSE_I, args=[arg_0, ConstInt(0), arg_2])
            self.optimizer.send_extra_operation(newop)
            self._rule_fired_jit_choose_i[4] += 1
            return
        # choose_self_right: jit_choose_i(x, y, x) => jit_choose_i(x, y, 1)
        if self._eq(arg_2, b_arg_2, arg_0, b_arg_0):
            if b_arg_0.is_bool():
                newop = self.replace_op_with(op, rop.JIT_CHOOSE_I, args=[arg_0, arg_1, ConstInt(1)])
                self.optimizer.send_extra_operation(newop)
                self._rule_fired_jit_choose_i[5] += 1
                return
        return self.emit(op)
