...
class MetaInterp(object):
    ...
    def free_regs(self, num_regs, num_consts):
        # clear registers to free memory (only necessary for refs?)
        for i in range(0, num_regs + num_consts):
            self.regs[self.allocated_regs + i] = valueapi.NoValue 
    ...
    def initialize_state_from_start(self, original_boxes):
        self.newframe(self.jitdriver_sd.mainjitcode)
        self.jitcodes.append(self.jitdriver_sd.mainjitcode)
        self.setup_call(original_boxes)

    def newframe(self, jitcode):
        reg_offset = self.alloc_regs(jitcode.num_regs_i()
                                               + jitcode.num_regs_r()
                                               + jitcode.num_regs_f())

        self.registers_offset_i = reg_offset
        self.registers_offset_r = reg_offset + jitcode.num_regs_i()
        self.registers_offset_f = reg_offset + jitcode.num_regs_i() + jitcode.num_regs_r()
    
    def finishframe(self, return_value):
        jitcode = self.jitcodes.pop()

        # free allocated registers
        regs = jitcode.num_regs_i() + jitcode.num_regs_r() + jitcode.num_regs_f()
        consts = len(jitcode.constants_i) + len(jitcode.constants_r) + len(jitcode.constants_f)
        self.free_regs(regs, consts)
        ...
        if self.jitcodes:
            ...
            self.registers_offset_i = self.allocated_regs - offset
            self.registers_offset_r = self.allocated_regs - offset + jitcode.num_regs_i()
            self.registers_offset_f = self.allocated_regs - offset + jitcode.num_regs_i() + jitcode.num_regs_r()

            self.pc = self.call_stack.pop()
            target_index = ord(jitcode.code[self.pc-1])
            self.set_reg_i(target_index, return_value)
    ...
    def interpret(self):
        ...
        while True:
            controlflow, return_value = self.metainterp_sd.run_one_step(self)
    ...
    def get_reg_i(self, index):
        return self.regs[self.registers_offset_i + index]
    
    def get_reg_r(self, index):
        return self.regs[self.registers_offset_r + index]
    
    def get_reg_f(self, index):
        return self.regs[self.registers_offset_f + index]
    
    def set_reg_i(self, index, value):
        self.regs[self.registers_offset_i + index] = value
    
    def set_reg_r(self, index, value):
        self.regs[self.registers_offset_r + index] = value
    
    def set_reg_f(self, index, value):
        self.regs[self.registers_offset_f+ index] = value
    ...
    def generate_guard(self, opnum, box=None, extraarg=None, resumepc=-1):
        ...
        if valueapi.is_constant(box):    # no need for a guard
            return
        guard_op = self.history.record1(opnum, box, None) # TODO
        
        saved_pc = self.pc
        if resumepc >= 0:
            self.pc = resumepc
        # TODO capture resume data expects MIFrames
    ...
    def get_list_of_active_boxes(self, in_a_call, new_array, encode, after_residual_call=False):
        ...
        if in_a_call:
            ...
            index = ord(self.jitcodes[-1].code[self.pc - 1])
            if argcode == 'i':
        ...
        assert ord(self.jitcodes[-1].code[pc]) == self.metainterp_sd.op_live
        if not we_are_translated():
            assert pc in self.jitcodes[-1]._startpoints
        offset = decode_offset(self.jitcodes[-1].code, pc + 1)
    ...
    def opimpl_int_add(self, a, b):
        ...
        if not (constness_a and constness_b):
            res_box = self.history.record2(rop.INT_ADD, a, b, res)
        else:
    ...
    def opimpl_int_sub(self, a, b):
        ...
        res = value_a - value_b
        if not (constness_a and constness_b):
            res_box = self.history.record2(rop.INT_SUB, a, b, res)
        else:
            res_box = valueapi.create_const(res)
        return res_box, CONTINUE_EXECUTE, valueapi.NoValue
    ...
    def opimpl_int_mul(self, a, b):
        ...
        res = value_a * value_b
        if not (constness_a and constness_b):
            res_box = self.history.record2(rop.INT_MUL, a, b, res)
        else:
            res_box = valueapi.create_const(res)
        return res_box, CONTINUE_EXECUTE, valueapi.NoValue
    ...
    def opimpl_goto_if_not_int_gt(self, a, b, target, orgpc):
        ...
        res = value_a > value_b
        if not (constness_a and constness_b):
            res_box = self.history.record2(rop.INT_GT, a, b, res)
            if res:
    ...
    def opimpl_goto_if_not_int_lt(self, a, b, target, orgpc):
        ...
        res = value_a < value_b
        if not (constness_a and constness_b):
            res_box = self.history.record2(rop.INT_LT, a, b, res)
            if res:
                opnum = rop.GUARD_TRUE
            else:
                opnum = rop.GUARD_FALSE
    ...
    def opimpl_goto_if_not_int_eq(self, a, b, target, orgpc):
        ...
        res = value_a == value_b
        if not (constness_a and constness_b):
            res_box = self.history.record2(rop.INT_EQ, a, b, res)
            if res:
                opnum = rop.GUARD_TRUE
            else:
                opnum = rop.GUARD_FALSE
    ...
    def opimpl_int_return(self, b):
        # TODO
        controlflow, return_value = self.finishframe(b)
    ...
    def opimpl_strgetitem(self, strbox, indexbox):
        ...
        value_i, constness_i = valueapi.get_value_int_and_constness(indexbox)
        res = ord(s.chars[value_i])
        if (valueapi.is_constant(strbox) and constness_i):
            return valueapi.create_const(res), CONTINUE_EXECUTE, valueapi.NoValue
        return self.history.record2(rop.STRGETITEM, strbox, indexbox, res), CONTINUE_EXECUTE, valueapi.NoValue
    
    @arguments("box", "descr", "orgpc")
    def opimpl_switch(self, valuebox, switchdict, orgpc):
        search_value, constness = valueapi.get_value_int_and_constness(valuebox)
        ...
        try:
            ...
            assert False
        if not constness:
            promoted_box = valueapi.create_const(search_value)
            self.generate_guard(rop.GUARD_VALUE, valuebox, resumepc=orgpc)
            self.replace_box(valuebox, promoted_box)
        self.pc = target
        return None, CONTINUE_EXECUTE, valueapi.NoValue

    def not_implemented(self, *args):
        name = self.metainterp_sd.opcode_names[ord(self.jitcodes[-1].code[self.pc])]
...
def miniinterp_staticdata(metainterp_sd, cw):
    ...
    def run_one_step(self):
        while True:
            ...
            pc = self.pc
            op = ord(self.jitcodes[-1].code[pc])
            if not we_are_translated():
    ...
    def handler(self, position):
        ...
        for argtype in argtypes:
            if argtype == "box":     # a box, of whatever type
                ...
                value = self.metainterp_sd.opcode_descrs[index]
                if argtype == "jitcode":
        ...
        if resultbox is not None:
            target_index = ord(self.jitcodes[-1].code[self.pc-1])
            self.set_reg_i(target_index, resultbox) # TODO only ints supported so far
        return controlflow, return_value
    #
    if not hasattr(MetaInterp, 'opimpl_' + name):
        return MetaInterp.not_implemented.im_func
    unboundmethod = getattr(MetaInterp, 'opimpl_' + name).im_func
