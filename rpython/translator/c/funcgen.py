...
class StaticNullPtrDereference(Exception):
    def __init__(self, message="static NULL deref"):
...
class FunctionCodeGenerator(object):
        ...
        try:
            ...
            if "\n" not in line:
                ...
                for line in line.splitlines():
                    yield line
        except StaticNullPtrDereference as e:
            # someone tries to directly dereference a NULL constant. This is
            # invalid in C, but may be generated by dead code.
            yield "abort(); /* %s */" % e.message
    ...
    def generic_call(self, FUNC, fnexpr, args_v, v_result, targets=None):
        ...
        if fnexpr == 'NULL':
            raise StaticNullPtrDereference("call to NULL")
        else:
    ...
    def OP_GETARRAYSIZE(self, op):
        ...
        if isinstance(ARRAY, FixedSizeArray):
            ...
            arr_expr = self.expr(op.args[0])
            res_expr = self.expr(op.result)
            if arr_expr == 'NULL':
                raise StaticNullPtrDereference('NULL->length')
            return '%s = %s->length;' % (res_expr, arr_expr)
    ...
    def OP_GETARRAYITEM(self, op):
        ...
        if ptr == 'NULL':
            raise StaticNullPtrDereference('= RPyBareItem(NULL, %s)' % index)
    ...
    def interior_expr(self, args, rettype=False):
        ...
        for i, arg in enumerate(args[1:]):
            ...
            if arg.concretetype is Void:
                ...
                if expr == 'NULL':
                    raise StaticNullPtrDereference('NULL[%s]' % indexexpr)
                if i == 0:
    ...
    def OP_DIRECT_ARRAYITEMS(self, op):
        ...
        if items == 'NULL':
            raise StaticNullPtrDereference('NULL->items')
