...
handle object."""
        w_key = self.descr_handle_get(space)
        space.audit("winreg.PyHKEY.Detach", [w_key])
...
def hkey_to_w_int(w_hkey, space):
    if isinstance(w_hkey, W_HKEY):
        return w_hkey.descr_handle_get(space)
    else:
        assert space.isinstance_w(w_hkey, space.w_int)
        return w_hkey
...
The docs imply key must be in the HKEY_USER or HKEY_LOCAL_MACHINE tree"""
    ...
    hkey = hkey_w(w_hkey, space)
    subkey = space.realunicode_w(w_subkey)
    filename = space.realunicode_w(w_filename)
    space.audit("winreg.LoadKey",
                [hkey_to_w_int(w_hkey, space), w_subkey, w_filename])
...
privilege. This function passes NULL for security_attributes to the API."""
    hkey = hkey_w(w_hkey, space)
    filename = space.realunicode_w(w_filename)
    space.audit("winreg.SaveKey", [hkey_to_w_int(w_hkey, space), w_filename])
    with rffi.scoped_unicode2wcharp(filename) as wide_filename:
        c_filename = rffi.cast(rffi.CWCHARP, wide_filename)
...
def SetValue(space, w_hkey, w_subkey, w_type, w_value):
    """
...
But the underlying API call doesn't return the type: Lame, DONT USE THIS!!!"""
    ...
    space.audit("winreg.QueryValue",
                [hkey_to_w_int(w_hkey, space), w_subkey, space.w_None])
...
def SetValueEx(space, w_hkey, w_value_name, w_reserved, w_type, w_value):
    """
...
the configuration registry.  This helps the registry perform efficiently."""
    typ = space.int_w(w_type)
    value_name = space.realunicode_w(w_value_name)
    hkey = hkey_w(w_hkey, space)
    buf, buflen = convert_to_regdata(space, w_value, typ)
    space.audit("winreg.SetValue",
                [hkey_to_w_int(w_hkey, space), w_value_name, w_type, w_value])
    try:
...
value_name is a string indicating the value to query"""
    hkey = hkey_w(w_hkey, space)
    space.audit("winreg.QueryValue",
                [hkey_to_w_int(w_hkey, space), space.w_None, w_subkey])
    if space.is_w(w_subkey, space.w_None):
        subkey = None
    else:
        subkey = space.utf8_w(w_subkey).decode('utf8')
...
If the function fails, an exception is raised."""
    subkey = space.realunicode_w(w_subkey)
    hkey = hkey_w(w_hkey, space)
    space.audit("winreg.CreateKey", [hkey_to_w_int(w_hkey, space), w_subkey,
                                     space.newint(r_uint(rwinreg.KEY_WRITE))])
    with rffi.scoped_unicode2wcharp(subkey) as wide_subkey:
        c_subkey = rffi.cast(rffi.CWCHARP, wide_subkey)
        with lltype.scoped_alloc(rwinreg.PHKEY.TO, 1) as rethkey:
            ...
            res = W_HKEY(space, rethkey[0])
            space.audit("winreg.OpenKey/result", [res.descr_handle_get(space)])
            return res


@unwrap_spec(reserved=int)
def CreateKeyEx(space, w_key, w_sub_key, reserved=0, w_access=None):
    """key = CreateKey(key, sub_key) - Creates or opens the specified key.
...
If the function fails, an exception is raised."""
    if w_access is None:
        w_access = space.newint(r_uint(rwinreg.KEY_WRITE))
    access = space.c_uint_w(w_access)
    sub_key = space.realunicode_w(w_sub_key)
    hkey = hkey_w(w_key, space)
    space.audit("winreg.CreateKey",
                [hkey_to_w_int(w_key, space), w_sub_key, w_access])
...
is removed.  If the method fails, an EnvironmentError exception is raised."""
    ...
    space.audit("winreg.DeleteKey",
                [hkey_to_w_int(w_hkey, space), w_subkey, space.newint(0)])
    with rffi.scoped_unicode2wcharp(subkey) as wide_subkey:
        c_subkey = rffi.cast(rffi.CWCHARP, wide_subkey)
...
value is a string that identifies the value to remove."""
    subkey = space.realunicode_w(w_subkey)
    hkey = hkey_w(w_hkey, space)
    space.audit("winreg.DeleteValue", [hkey_to_w_int(w_hkey, space), w_subkey])
    with rffi.scoped_unicode2wcharp(subkey) as wide_subkey:
        c_subkey = rffi.cast(rffi.CWCHARP, wide_subkey)
...
@unwrap_spec(reserved=int)
def OpenKey(space, w_key, w_sub_key, reserved=0, w_access=None):
    """
...
If the function fails, an EnvironmentError exception is raised."""
    if w_access is None:
        w_access = space.newint(r_uint(rwinreg.KEY_READ))
    access = space.c_uint_w(w_access)
    hkey = hkey_w(w_key, space)
...
def EnumValue(space, w_hkey, w_index):
...
data_type is an integer that identifies the type of the value data."""
    ...
    space.audit("winreg.EnumValue", [hkey_to_w_int(w_hkey, space), w_index])
    null_dword = lltype.nullptr(rwin32.LPDWORD.TO)
...
def EnumKey(space, w_hkey, w_index):
...
raised, indicating no more values are available."""
    hkey = hkey_w(w_hkey, space)
    index = space.int_w(w_index)
    space.audit("winreg.EnumKey", [hkey_to_w_int(w_hkey, space), w_index])
    null_dword = lltype.nullptr(rwin32.LPDWORD.TO)
...
A long integer that identifies when the key was last modified (if available)
 as 100's of nanoseconds since Jan 1, 1600."""
    hkey = hkey_w(w_hkey, space)
    space.audit("winreg.QueryInfoKey", [hkey_to_w_int(w_hkey, space)])
...
If the function fails, an EnvironmentError exception is raised."""
    hkey = hkey_w(w_hkey, space)
    space.audit("winreg.ConnectRegistry", [w_machine, hkey_to_w_int(w_hkey, space)])
...
def ExpandEnvironmentStrings(space, w_source):
    ...
    space.audit("winreg.ExpandEnvironmentStrings", [w_source])
    try:
...
def DisableReflectionKey(space, w_key):
    ...
    subkeys."""
    hkey = hkey_w(w_key, space)
    space.audit("winreg.DisableReflectionKey", [hkey_to_w_int(w_key, space)])
...
def EnableReflectionKey(space, w_key):
    ...
    subkeys."""
    hkey = hkey_w(w_key, space)
    space.audit("winreg.EnableReflectionKey", [hkey_to_w_int(w_key, space)])
...
def QueryReflectionKey(space, w_key):
    ...
    hkey = hkey_w(w_key, space)
    space.audit("winreg.QueryReflectionKey", [hkey_to_w_int(w_key, space)])
...
@unwrap_spec(reserved=int)
def DeleteKeyEx(space, w_key, w_sub_key, w_access=None, reserved=0):
    ...
    if w_access is None:
        w_access = space.newint(r_uint(rwinreg.KEY_WOW64_64KEY))
    access = space.c_uint_w(w_access)
    sub_key = space.realunicode_w(w_sub_key)
    hkey = hkey_w(w_key, space)
    space.audit("winreg.DeleteKey",
