# spaceconfig = {"usemodules" : ["_weakref"]}
...
def test_callback_cannot_ressurect():
    import _weakref, gc
    class A(object):
        pass
    a = A()
...
def test_correct_weakrefcount_after_death():
    import _weakref, gc
    class A(object):
        pass
    a = A()
...
def test_generator_weakrefable():
    import _weakref, gc
    def f(x):
        for i in range(x):
            yield i
...
def test_weakref_subclass_with_del():
    import _weakref, gc
    class Ref(_weakref.ref):
        def __del__(self):
            b.a = 42
...
def test_repr():
    ...
    for kind in ('ref', 'proxy'):
        ...
        print(s)
        if kind == 'ref':
            assert s.startswith('<weakref.ReferenceType at ')
        else:
