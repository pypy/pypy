# spaceconfig = {"usemodules": ["binascii", "imp", "time", "struct", "_cffi_backend"]}
...
def test_find_frozen():
    import _imp
    assert _imp.find_frozen('hello.what.now') is None


def test_get_frozen_object_no_such_name():
    import _imp
...
def test_get_frozen_object_issue105979():
...
def test_get_frozen_object_with_data():
    import _imp, marshal
    co = compile("marker = 42", "x.py", "exec")
...
def test_get_frozen_object_bad_type():
    import _imp
    raises(TypeError, _imp.get_frozen_object, "x", 42)


def test_get_frozen_object_not_a_code_object():
    import _imp, marshal
...
def test_path_importer_cache():
    ...
    lib_pypy = os.path.abspath(
        os.path.join(__file__, "..", "..", "..", "..", "..", "lib_pypy")
    )
...
def test_builtin_reimport_mess():
...
def test_check_hash_based_pycs():
    import _imp
