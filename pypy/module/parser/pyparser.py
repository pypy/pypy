...
def build_node_tree(space, w_tuple, parser):
    ...
    return build_node_children(space, type, tup_w, node_state, parser)

def build_node_children(space, type, tup_w, node_state, parser):
    ...
    for i in range(1, len(tup_w)):
        ...
        if type < 256:  # Terminal node
            ...
            child = build_node_children(space, type, subtup_w, node_state, parser)
...
def tuple2st(space, w_sequence):
    ...
    parser = pyparse.PythonParser(space)
    tree = build_node_tree(space, w_sequence, parser)
