...
# -------------------------
# Tests ported from CPython
# -------------------------

def test_re_groupref_exists_validation_bug():
    # CPython gh-98740: validation of conditional expressions in RE
    import re
    for i in range(256):
...
def test_sre_template_invalid_group_index():
    # CPython gh-106524: crash in _sre.template() with invalid indices
    import _sre
...
def test_bug_gh106052():
    # CPython gh-106052 and gh-100061: possessive quantifiers with backtracking sub-patterns
    import re
    ...
    assert re.match('(?:ab?c){1,3}+', 'aca').span() == (0, 2)
    # CPython gh-100061: restore input pointer after possessive sub-pattern
