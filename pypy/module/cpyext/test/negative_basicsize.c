 ...
 * instance is allocated; PyPy now raises SystemError from PyType_Ready().
 *
 ...
 *   expected:               base_basicsize + extra (what CPython's
 *                           PyType_FromMetaclass would produce)
 */
#include <Python.h>
...
make_meta(PyObject *self, PyObject *args)
{
    ...
    return Py_BuildValue("{s:n,s:n,s:n,s:n,s:N}",
                         ...
                         "base_basicsize", PyType_Type.tp_basicsize,
                         "expected", PyType_Type.tp_basicsize + extra,
                         "meta", (PyObject *) tp);
}
