...
class AppTestCall(AppTestCpythonExtensionBase):
    ...
    def test_set_bases(self):
        # issue 4826
        module = self.import_extension('foo', [
            ('set_bases', 'METH_VARARGS',
            '''
                PyObject *obj = PyTuple_GetItem(args, 0);
                PyObject *value = PyTuple_GetItem(args, 1);
                PyObject *name = PyUnicode_FromString("__bases__");
                PyObject *descr = _PyType_Lookup((PyTypeObject *)obj, name);
                int ret = 0;
                if (descr == NULL) {
                    ret = PyType_Type.tp_setattro(obj, name, value);
                    if (ret < 0) return NULL;
                    ret += 100;
                }
                else {
                    PyObject *result = PyObject_CallMethod(descr, "__set__", "OO", obj, value);
        ...
        # Now via a C call
        ret = module.set_bases(C, (A,))
        assert C.__bases__ == (A,)
