...
def HPy_Power(space, state, ctx, h1, h2, h3):
    w_o1 = state.handles.deref(h1)
    w_o2 = state.handles.deref(h2)
    w_o3 = state.handles.deref(h3)
...
def HPyBool_FromLong(space, state, ctx, value):
    if widen(value) != 0:
        return state.handles.new(space.w_True)
    return state.handles.new(space.w_False)
