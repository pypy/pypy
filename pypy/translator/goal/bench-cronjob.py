...
def main(backends=[]):
    if backends == []:  #_ prefix means target specific option
        ...
        backends = """llvm@c@c--gc=framework@c--_thread@c--stackless@c--gc=framework--cc=c++@c--cc=c++@c--objspace-std-withstrdict@c--profopt='-c "from richards import *;main(iterations=1)"'""".split('@')
