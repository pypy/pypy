...
class TestBuiltinCode:
    def test_signature(self):
        ...
        code = gateway.BuiltinCode(c, unwrap_spec=[gateway.ObjSpace,
                                                   ...
                                                   gateway.W_Root,
                                                   'args_w'])
        assert code.signature() == argument.Signature([u'x', u'y'], u'hello', None)
        def d(self, w_boo):
            pass
        code = gateway.BuiltinCode(d, unwrap_spec= ['self',
                                                   gateway.W_Root], self_type=gateway.Wrappable)
        assert code.signature() == argument.Signature([u'self', u'boo'], None, None)
        ...
        code = gateway.BuiltinCode(e, unwrap_spec=[gateway.ObjSpace,
                                                   ...
                                                   gateway.W_Root,
                                                   gateway.Arguments])
        assert code.signature() == argument.Signature([u'x', u'y'], u'args', u'keywords')
